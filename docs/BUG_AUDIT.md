# SkyN3t 2.0 — Bug Audit (RESOLVED)

All 54 confirmed findings fixed (manual high-severity batches 1-4 + 11-agent subsystem fan-out).

54 confirmed of 55 claims. Status: [ ] open  [x] fixed

- [x] **HIGH** `agents_ext` — EnvScanner skip-dir check matches ancestor path components, silently scans zero files
...
- [x] **HIGH** `studio` — Reviewer 'go' verdict bypasses the proof-run, shipping broken/empty builds as go
...
- [x] **HIGH** `agents_verify` — Reviewer returns GO for a project with zero non-empty source files
...
- [x] **HIGH** `memory_rag` — _call_engine targets methods RagEngine does not have and passes a dict where text is expected
...
- [x] **HIGH** `cortex` — Auto-applied TUNING proposals have no effect on the running system (overrides dict is read by nothing)
...
- [x] **HIGH** `web` — Cortex proposal decide uses wrong identity field (id vs proposal_id) — approve/reject always 404/no-op
...
- [x] **HIGH** `web` — Browser WebSocket clients can never connect when auth_token is set (SPA omits ?token=)
...
- [x] **HIGH** `cli_config` — CLI subprocess is orphaned (leaked) on timeout in _cli
...
- [x] **HIGH** `cli_config` — `skyn3t snapshot` always writes an empty snapshot
...
- [x] **HIGH** `agents_gen` — CodeImproverAgent overwrites good files with stub prose on CLI degradation
...
- [x] **HIGH** `memory_rag` — ExperienceIngestor imports non-existent class RAGEngine — auto-ingestion silently dead
...
- [x] **HIGH** `cli_config` — `studio approve`/`reject` silently lose the decision (no-op)
...
- [x] **HIGH** `core` — Idempotency only dedups AFTER completion — concurrent duplicate submits both execute fully
...
- [x] **HIGH** `core` — Failed results are cached under idempotency_key forever, permanently poisoning the key
...
- [x] **HIGH** `studio` — Stage submission has no timeout despite the 'stage_timeout' contract; a hung agent hangs the whole build forever
...
- [x] **HIGH** `agents_gen` — CodeAgent overwrites runnable scaffold files with stub prose when CLI backend degrades
...
- [x] **HIGH** `agents_verify` — Reward-hacking checks #1 and #4 are dead in the real pipeline (claims_success never true)
...
- [x] **HIGH** `intelligence` — Docker timeout silently re-runs the command inline with no sandbox (double execution + isolation loss)
...
- [x] **HIGH** `core` — BaseAgent.run mutates shared status with no concurrency guard; orchestrator routes same agent concurrently
...
- [x] **HIGH** `cortex` — AutonomousLoop.stop()/teardown never cancels in-flight build tasks
...
- [x] **HIGH** `cortex` — ReviewWatcher dispatches a repair orchestrator task regardless of the proposal triage/gating decision
...
- [x] **HIGH** `integrations_obs` — docker_available() runs blocking subprocess/SDK calls inside the async run() path, stalling the event loop
...
- [x] **MEDIUM** `studio` — proof_run never uses the sandbox under the default execution_backend='auto', silently degrading all proofs to local file checks
...
- [x] **MEDIUM** `agents_ext` — Static deploy falls back to serving the entire project dir (incl. .env / source) on success
...
- [x] **MEDIUM** `core` — Unbounded growth of _results and _idempotency dicts (memory leak)
...
- [x] **MEDIUM** `studio` — Blocking proof_run (rglob + py_compile of every .py + docker ping) runs on the event loop, including N times under asyncio.gather
...
- [x] **MEDIUM** `agents_gen` — WriterAgent writes stub prose as README on CLI degradation
...
- [x] **MEDIUM** `agents_verify` — test_author generates a syntactically broken / injectable acceptance test from unsanitized LLM criteria
...
- [x] **MEDIUM** `agents_ext` — summarize_commits top_paths is always empty — list-commits API never returns per-commit files
...
- [x] **MEDIUM** `intelligence` — Non-atomic JSON persistence can corrupt and silently wipe the scoreboard/tournament on crash
...
- [x] **MEDIUM** `cortex` — USD budget guard is ineffective: spend is recorded only after a build completes and there is no per-build pre-check
...
- [x] **MEDIUM** `web` — Concurrent send_text on a single WebSocket: hub fan-out races the per-connection history priming
...
- [x] **MEDIUM** `cli_config` — Per-build budget cap never resets across builds in long-lived process (web mode)
...
- [x] **MEDIUM** `cli_config` — `--no-critic` permanently mutates the cached settings singleton for the process
...
- [x] **MEDIUM** `cli_config` — OpenRouter backend crashes instead of degrading on HTTP error or malformed response
...
- [x] **MEDIUM** `integrations_obs` — Outbound delivery text and event preview are never scrubbed of secrets
...
- [x] **MEDIUM** `integrations_obs` — Per-build cost attribution double-counts and is wrong under overlapping builds
...
- [x] **MEDIUM** `intelligence` — Inline subprocess not reaped after kill on timeout (resource/transport leak)
...
- [x] **MEDIUM** `agents_verify` — Contract verifier marks host files outside the project as satisfied (absolute path / traversal)
...
- [x] **MEDIUM** `memory_rag` — LessonHygiene.sweep scans highest-scored lessons but only low/negative-scored ones are stale
...
- [x] **MEDIUM** `web` — Fire-and-forget build task can be garbage-collected mid-run; exceptions silently lost
...
- [x] **MEDIUM** `studio` — Best-of-N candidate worktrees can leak when a trajectory or proof raises / gather is interrupted
...
- [x] **LOW** `agents_gen` — node_express scaffold declares a test script pointing at a non-existent file
...
- [x] **LOW** `integrations_obs` — NL schedule 'every N minutes' with N>59 produces a cron that silently fires only at minute 0
...
- [x] **LOW** `memory_rag` — relevant_lessons omits helpful/hurt, disabling hit-rate logic in effective_score
...
- [x] **LOW** `studio` — proof_run mutates the delivered project by writing __pycache__/.pyc during the syntax check
...
- [x] **LOW** `agents_verify` — boot_verifier treats any import timeout as a successful boot (false pass)
...
- [x] **LOW** `agents_ext` — Generic key=value redaction also deletes the key name, can corrupt non-secret config text
...
- [x] **LOW** `agents_verify` — Timed-out subprocesses are killed but never reaped (resource leak)
...
- [x] **LOW** `core` — Handler exceptions silently swallowed — no logging, no failure signal
...
- [x] **LOW** `memory_rag` — grade_lesson read-modify-write of counters is racy under concurrent grading
...
- [x] **LOW** `agents_gen` — CodeImproverAgent can rewrite files in the current working directory when no worktree is provided
...
- [x] **LOW** `agents_verify` — consistency_reviewer pollutes local_tops with leaf module names, suppressing real broken-import detection
...
- [x] **LOW** `web` — metrics history scan rebuilds full per-type counts on every /api/metrics call (O(history) each request)
