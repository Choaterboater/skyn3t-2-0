...
| Out-of-band `studio approve/reject` reattaching to live builds | 🟡 | Records a decision event today; live reattach pending (see STATUS gaps). |
| Docker-sandboxed proof-run | 🟡 | `execution_backend=docker`; falls back to inline without the Docker SDK/daemon. |
