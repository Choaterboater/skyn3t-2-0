...
| GitHub→RAG→skill distill | **wired, 1 skill/repo** | `domain ingest <url>` makes one skill per repo |
...
- **B2 — Reflector-driven learning proposals.** Collect per-stage transcripts; on
  ...
  not yet auto-apply.)
- **B3 — Per-file skills ingest.** Extend the GitHub ingest to list+fetch a repo's `*.md`
  and distill one skill per file, so e.g. `addyosmani/agent-skills` becomes 24 retrievable
  advisory skills (knowledge B1/B2 feed on).
