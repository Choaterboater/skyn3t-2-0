...
DEFAULT_CASES_RAW = [
    ...
    ("todo-cli", "a python command-line to-do list that saves tasks to a file", "python_cli"),
    ("notes-api", "a FastAPI service to create and list short text notes", "fastapi"),
    ("weather-spa", "a React app that shows a 5-day weather forecast for a city", "react_vite"),
    ("expense-tracker", "a Flask web app to track monthly expenses by category", ""),
]
...
def diff_runs(before: BenchRun, after: BenchRun, *,
    ...
    A regression = a go->no_go flip OR a score drop beyond the threshold (on a
    case that stayed go). An improvement = a no_go->go flip."""
    ...
    for a in after.results:
        ...
        if b is None:
            per_case.append({"case_id": a.case_id, "status": "new",
        ...
        if b.verdict == "go" and a.verdict == "no_go":
            regressions.append({**entry, "kind": "verdict"})
        elif b.verdict == "no_go" and a.verdict == "go":
            improvements.append({**entry, "kind": "verdict"})
        elif b.verdict == "go" and a.verdict == "go" and score_delta <= -score_regress_threshold:
            regressions.append({**entry, "kind": "score"})

    return {
        "mean_score_delta": round(after.summary.get("mean_score", 0.0)
                                  - before.summary.get("mean_score", 0.0), 2),
        "mean_intent_delta": round(after.summary.get("mean_intent", 0.0)
                                   - before.summary.get("mean_intent", 0.0), 2),
        "go_rate_delta": round(after.summary.get("go_rate", 0.0)
                               - before.summary.get("go_rate", 0.0), 4),
...
def gate_change(delta: dict[str, Any], *, min_mean_score_delta: float = 0.0,
    ...
    """Whether a change should be ACCEPTED based on the measured delta. Reject on
    any verdict regression (unless allowed) or a mean-score delta below the bar.
    ...
    msd = float(delta.get("mean_score_delta", 0.0))
    if msd < min_mean_score_delta:
        reasons.append(f"mean score delta {msd} < required {min_mean_score_delta}")
