...
def test_improve_context_includes_design_md(tmp_path):
    ...
    (project / "DESIGN.md").write_text(
        ...
        "## DESIGN TOKENS — base theme 'paper'\n  --accent: #123456;\n",
        encoding="utf-8",
    )
    bus = EventBus()
    orchestrator = _FakeOrchestrator()
    ...
    assert outcome.status == "completed"
    repo_map = orchestrator.submitted[0].payload["repo_map"]
    assert repo_map.startswith(
        "The project's locked design direction (DESIGN.md) — keep styling "
        "consistent with it unless the goal explicitly restyles:"
    )
    assert "--accent: #123456;" in repo_map


def test_improve_context_without_design_md_stays_plain(tmp_path):
    settings = _settings(tmp_path)
    ...
    assert outcome.status == "completed"
    repo_map = orchestrator.submitted[0].payload["repo_map"]
    assert "locked design direction" not in repo_map
